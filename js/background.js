...
function checkRedirects(details) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (result.isMatch) {
      ...
      // Preserve the all the query parameters for redirection
      return {
        redirectUrl:
          result.redirectTo +
...
function monitorChanges(changes, namespace) {
  ...
  if (changes.enableNotifications) {
    log(
      "notifications setting changed to " + changes.enableNotifications.newValue
    );
    ...
    storageArea.set(request, function (a) {
      if (chrome.runtime.lastError) {
        if (
          ...
          log(
            "Redirects failed to save as size of redirects larger than allowed limit per item by Sync"
          );
          sendResponse({
            message:
    ...
    chrome.storage.local.set(
      ...
      function () {
        if (request.isSyncEnabled) {
          ...
          log(
            "storageArea size for sync is 5 MB but one object (redirects) is allowed to hold only " +
              storageArea.QUOTA_BYTES_PER_ITEM / 1000000 +
              " MB, that is .. " +
              storageArea.QUOTA_BYTES_PER_ITEM +
              " bytes"
          );
          chrome.storage.local.getBytesInUse("redirects", function (size) {
            ...
            if (size > storageArea.QUOTA_BYTES_PER_ITEM) {
              log(
                "size of redirects " +
                  size +
                  " is greater than allowed for Sync which is " +
                  storageArea.QUOTA_BYTES_PER_ITEM
              );
              ...
              sendResponse({
                message:
                  "Sync Not Possible - size of Redirects larger than what's allowed by Sync. Refer Help page",
              });
            } else {
              chrome.storage.local.get(
                ...
                function (obj) {
                  ...
                  if (obj.redirects.length > 0) {
                    ...
                    log(
                      "No redirects are setup currently in Local, just enabling Sync"
                    );
                    sendResponse({
                      message: "sync-enabled",
                    });
          ...
          storageArea = chrome.storage.local;
          log(
            "storageArea size for local is " +
              storageArea.QUOTA_BYTES / 1000000 +
              " MB, that is .. " +
              storageArea.QUOTA_BYTES +
              " bytes"
          );
...
function sendNotifications(redirect, originalUrl, redirectedUrl) {
  ...
  let icon = isDarkMode()
    ? "images/icon-dark-theme-48.png"
    : "images/icon-light-theme-48.png";
